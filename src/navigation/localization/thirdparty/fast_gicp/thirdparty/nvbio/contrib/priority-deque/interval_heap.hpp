...
//  By keeping track of where I started, I can roll back all changes.
  Offset index = origin;
#if (__cplusplus >= 201103L)  //  C++11
  Value langnav = std::move_if_noexcept(*(first + index));
#endif
...
//  If compare throws, heap property cannot be verified or enforced.
      try { //  This try-catch block ensures no element is left in langnav.
        if (compare(*(first + child + (left_bound ? 2 : 0)),
                    *(first + child + (left_bound ? 0 : 2))))
          child += 2;
      } catch (...) {
#if (__cplusplus >= 201103L)  //  C++11
//  Pull the moving element out of langnav, to avoid leaks.
        *(first + index) = std::move_if_noexcept(langnav);
#endif
        throw;
      }
#if (__cplusplus >= 201103L)  //  C++11
...
//  Pull the moving element out of langnav.
#if (__cplusplus >= 201103L)
            *(first + index) = std::move_if_noexcept(langnav);
#endif
            throw;
...
#if (__cplusplus >= 201103L)  //  C++11
            *(first + index) = std::move_if_noexcept(*(first + child));
            *(first + child) = std::move_if_noexcept(langnav);
#else
            swap(*(first + index), *(first + child));
#endif
