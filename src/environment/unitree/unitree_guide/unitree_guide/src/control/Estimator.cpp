...
void Estimator::_initSystem(){
    ...
    /* ROS odometry publisher */
        ...
        _gt_pose_pub = _nh.advertise<nav_msgs::Odometry>("gt_pose", 1);
        _gz_state_sub = _nh.subscribe("gazebo/model_states", 1, &Estimator::StateCallback, this);
        _robot_name = "go1_gazebo"; //HARD CODING
    #endif  // COMPILE_WITH_MOVE_BASE
}

void Estimator::StateCallback(const gazebo_msgs::ModelStates& msg) {
    for(int i=0; i<msg.name.size(); i++) {
        if (msg.name[i] == _robot_name) {
            _currentTime = ros::Time::now();
            _gtPoseMsg.pose.pose = msg.pose[i];
            _gtPoseMsg.twist.twist = msg.twist[i];
            _gtPoseMsg.header.stamp = _currentTime;
            _gtPoseMsg.header.frame_id = "odom";
            _gtPoseMsg.child_frame_id = "base";
        }
    }
    _gt_pose_pub.publish(_gtPoseMsg);
}
